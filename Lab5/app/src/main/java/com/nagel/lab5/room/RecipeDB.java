...
public abstract class RecipeDB extends RoomDatabase {
    ...
    public static synchronized RecipeDB getInstance(Context context){
        if (instance == null){
            instance = Room.databaseBuilder(context.getApplicationContext(), RecipeDB.class,DB_NAME).fallbackToDestructiveMigration().addCallback(roomCallback).build();
        }return instance;
    }
    ...
    private static class PopulateDBAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... voids) {
            recipeDao.insert(new Recipe("Test", "test", "test",1));
            return null;
        }
    }
}
